// Mock de l’auth
jest.mock("../middleware/auth", () => ({
  protect: (req, res, next) => {
    req.user = { id: "fakeUserId" }; // utilisateur connecté fictif
    next();
  },
}));

// Mock du modèle Post
jest.mock("../models/Post", () => ({
  findById: jest.fn(),
}));

const request = require("supertest");
const app = require("../app");
const Post = require("../models/Post");

describe("POST /api/posts/:id/like", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it("like un post si pas encore liké", async () => {
    const fakePost = {
      _id: "post123",
      likes: [],
      save: jest.fn(),
    };

    Post.findById.mockResolvedValueOnce(fakePost);

    const res = await request(app).post("/api/posts/post123/like");

    expect(res.status).toBe(200);
    expect(res.body.liked).toBe(true);
    expect(fakePost.likes).toContain("fakeUserId");
  });

  it("retire un like si déjà liké", async () => {
    const fakePost = {
      _id: "post123",
      likes: ["fakeUserId"],
      save: jest.fn(),
    };

    Post.findById.mockResolvedValueOnce(fakePost);

    const res = await request(app).post("/api/posts/post123/like");

    expect(res.status).toBe(200);
    expect(res.body.liked).toBe(false);
    expect(fakePost.likes).not.toContain("fakeUserId");
  });

  it("renvoie 404 si le post n'existe pas", async () => {
    Post.findById.mockResolvedValueOnce(null);

    const res = await request(app).post("/api/posts/inexistant/like");

    expect(res.status).toBe(404);
    expect(res.body.message).toBe("Post introuvable");
  });

  it("renvoie 500 en cas d'erreur serveur", async () => {
    Post.findById.mockRejectedValueOnce(new Error("DB error"));

    const res = await request(app).post("/api/posts/post123/like");

    expect(res.status).toBe(500);
    expect(res.body.message).toBe("Erreur serveur");
  });
});
